...
class RouteImplData:
    ...
    def _routeboxes_per_second(self) -> float:
        completions_per_second = timedelta(seconds=1) / self.global_state.average_completion_time
    ...
    async def async_ticket_value(self, routebox_value: float) -> float:
        ...
        return self.return_on_route() - (self.average_route_completion_time / self.global_state.average_completion_time) * (self.global_state.average_completion_ev + (ROUTEBOX_CHANCE * routebox_value))
    


async def async_get_ticket_value(global_data: GlobalRunData, route_config: dict[Route, RouteImplData]) -> float:
    ...
    return (average_route_return - k * global_data.average_completion_ev) / (1 + k * ROUTEBOX_CHANCE)
